...
import config from "../cswap-keys/keys.json" assert {type: 'json'};
...
try {
  const privateKey = CardanoWasm.PrivateKey.from_normal_bytes(
    cbor.decodeFirstSync(config.paymentKey)
  );

  console.log(`PRIVATE KEY: ${privateKey.to_bech32()}`);

  const stakeKey = CardanoWasm.PrivateKey.from_normal_bytes(
    cbor.decodeFirstSync(config.stakeKey)
  );

  console.log(`STAKE KEY: ${stakeKey.to_bech32()}`);  
